...
class AssetServiceImplTest {
  ...
  void testGetAssetsByName() {
    ...
    when(assetRepository.findAllByNameContainingIgnoreCase("stock")).thenReturn(List.of(asset));

    List<Asset> result = assetService.getAssetsByName("stock");

    assertEquals(1, result.size());
    assertEquals("Stock A", result.get(0).getName());
    verify(assetRepository, times(1)).findAllByNameContainingIgnoreCase("stock");
  }
