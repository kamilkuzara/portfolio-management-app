...
public class AssetServiceImpl implements AssetService {
  ...
  public Asset updateAsset(Long id, AssetUpdateRequest assetUpdateRequest) {
    ...
    if (!assetOptional.isPresent()) return null;

    Asset asset = assetOptional.get();

    //        update the fields only if provided in the query
    if (assetUpdateRequest.getName() != null) asset.setName(assetUpdateRequest.getName());
    if (assetUpdateRequest.getType() != null) asset.setType(assetUpdateRequest.getType());
